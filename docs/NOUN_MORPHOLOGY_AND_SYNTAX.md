...
The default `Gender + singular` shorthand instead uses:

```text
<gender> <singular noun>
```

Both shorthand syntaxes reference the same `learned-shorthand` inference set. The second syntax can infer an ordinary noun with `articleMode: automatic` even though the typed answer itself contains no article. The syntax shape and the lexical facts it infers are separate concerns.
...
Several syntaxes can share one inference set. This lets study policy change once for several equivalent shorthand forms. The default article-based shorthand and gender-based shorthand both use the same `learned-shorthand` inference set.
...
Live preview selection has a different purpose from submission evaluation. A complete interpretation that produces candidates is preferred. If no complete interpretation produces candidates but a different syntax is still a viable partial parse, the preview keeps showing that partial parse rather than prematurely presenting a dead complete interpretation. Only when no viable partial parse remains does it show a complete zero-candidate interpretation.
...
`m specchio` follows the same learning policy through `Gender + singular`. It uses the same `learned-shorthand` inference set, so it is also wrong while only `o-i` is allowed.

Later, add `chio-chi` to `learned-shorthand`. Both shorthand inputs can now also produce a `chio-chi` candidate with base `spec`. That candidate matches the card, so either shorthand can be correct without changing the `specchio` card.
...
Browser persistence stores the inventory plus its synchronization timestamp in one localStorage value. The API stores the same complete synchronization state in one JSON file rather than coordinating separate card, morphology, and metadata files.
