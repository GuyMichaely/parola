...
Declension-rule names and inference-set names are references. They must be unique within their collections. Noun number availability is derived from the referenced rule's `forms`: both entries means both numbers; only one entry means singular-only or plural-only.
...
The service should reject a noun card whose `details.rule` does not match the name of a declension rule in the accompanying `nounMorphology.declensionRules` collection. It should also verify that the stored primary `italian` form is generated by that rule and the noun's `base`.
