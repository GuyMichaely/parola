    ...
    <section className="scope-panel" aria-label="Study scope">
      <label>
        ...
        <select value={mode} onChange={(event) => onMode(event.target.value as ScopeMode)}>
          ...
          <option value="only">Only selected types / sets / tags</option>
          <option value="exclude">Everything except selected types / sets / tags</option>
        </select>
      </label>
      {mode !== "all" && (
        <div className="set-picker">
          {options.length ? options.map((option) => (
            <button key={option.key} className={`${option.kind} ${selected.includes(option.key) ? "selected" : ""}`} aria-pressed={selected.includes(option.key)} onClick={() => onToggle(option.key)}>{option.kind === "type" ? "Type · " : option.kind === "tag" ? "Tag · " : "Set · "}{option.label}</button>
